...
- [x] 01-02-PLAN.md — Notification data + sole-writer chokepoint: repository (user_id-scoped) + service (idempotent notify, 7-event catalog, read methods)
...
- [x] 02-02-PLAN.md — Data access: cashoutRepository (idempotent create/read) + findByIdForUpdate lock helpers on wager/market repositories
- [x] 02-03-PLAN.md — resolveMarket payout fix: scale win payout by remaining (post-cashout) stake fraction + regression/scaled-payout tests

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 02-04-PLAN.md — Core transaction: wagerService.cashoutWager (lock-then-revalidate, formula, idempotent persistence, audit) + computation/validation tests
...
- [x] 03-03-PLAN.md — createMarket generalization (Over/Under + multiple-choice, 2–20 bound) + creation/validation tests (Wave 3)
- [x] 03-04-PLAN.md — placeWager generalization + IDOR/attack-vector tests (Wave 3)
- [x] 03-05-PLAN.md — resolveMarket N-outcome generalization + binary-assumption completeness audit + resolution tests (Wave 4)
- [x] 03-06-PLAN.md — Admin panel UI: type selector, Over/Under + dynamic option builder, resolve controls + additive CSS (Wave 5)
- [ ] 03-07-PLAN.md — Dashboard N-option rendering + phase-gate human verify against real Postgres (Wave 6)
...
- [x] 04-01-PLAN.md — Core rewrite: cancelWager (lock order fix, IDOR 404, cashout hard-block, 5% fee) + CANCEL_FEE_PERCENT config + regression-test update
...
| 4. Bet Cancellation v2 | 1/3 | In Progress|  |
</content>
